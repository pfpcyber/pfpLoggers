...
while(hasAutoStopped == false && getStreamingLatestValues == 0)
    ...
    newSampleT = newSampleT + newSamples;
    
    fprintf('New Samples(1): %d, startIndex(1): %d, newSampleT = %d.\n', newSamples, startIndex, newSampleT);
    % To increase the number of samples written to file change the value in
    % newSampleT == 250000, also change overviewBufferSize = 250000 in Pico3406DStreamingConfig.m;
    if (newSampleT == 250000)
        startIndex = 0;
        previousTotal = totalSamples;
        totalSamples = totalSamples + newSampleT;
        firstValuePosn = startIndex + 1;
        lastValuePosn = startIndex + newSampleT;
        % Printing to console can slow down acquisition - use for debug
        fprintf('New SampleT: %d, startIndex: %d total: %d.\n', newSampleT, startIndex, totalSamples);
